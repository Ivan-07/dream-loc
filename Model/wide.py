...
class IRFeatureFusion(nn.Module):
    ...
    def forward(self, r_idx, c_idx):
        cf = self.report_idx2cf[r_idx, c_idx].unsqueeze(1) # (bz, 1)
        ff = self.report_idx2ff[r_idx, c_idx].unsqueeze(1) # (bz, 1)
        fr = self.report_idx2fr[r_idx, c_idx].unsqueeze(1) # (bz, 1)
        sim = self.report_idx2sim[r_idx, c_idx].unsqueeze(1) # (bz, 1)
        cc = self.report_idx2cc[r_idx, c_idx].unsqueeze(1) # (bz, 1)
        feature_fusion = self.fusion(torch.cat([sim, cc, cf, ff, fr], dim=1))
        # (bz, 5) * (5, dim_dense) ---> (bz, dim_dense)
