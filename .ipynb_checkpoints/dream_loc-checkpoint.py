...
def cal_val_loss(model, data_loader):
    ...
    val_loss = []
    with torch.no_grad():
        for iteration, (report_idx, pos_code_idx, neg_code_idx) in enumerate(
                tqdm(data_loader, desc='val_loss', ncols=80)):
            report_idx, pos_code_idx, neg_code_idx = report_idx.to(device), pos_code_idx.to(device), neg_code_idx.to(
                device)
            loss = model(report_idx, pos_code_idx, neg_code_idx)
...
def train_val_model(train_dataset, val_dataset, val_loss_dataset, model_save_path,
    ...
    train_data_loader = DataLoader(dataset=train_set, batch_size=BATCH_SIZE, num_workers=1, pin_memory=True)
...
def main():
    ...
    # 模型保存路径
    model_save_path = f'{model_save_dir}/rmm_{RMM_DENSE_DIM}_irff_{IRFF_DENSE_DIM}_fusion_{FUSION_DENSE_DIM}_' \
                      f'batchsize_{BATCH_SIZE}_lr_{LR}_kpool_{K_MAX_POOL}.pth'
    ...
    if not JUST_TEST:
        train_val_model(train_dataset, val_dataset, val_loss_dataset, model_save_path,
                        matrix_bugidx2path_idx2valid_path_idx,
                        code_corpus_vec, code_word_idx2vec, snippet_idx2vec, code_snippet_idx2len, code_idx2len,
                        report_corpus_vec, report_word_idx2vec, report_word_idx2idf,
                        report_idx2cf, report_idx2ff, report_idx2fr, report_idx2sim, report_idx2tr, report_idx2cc)
